"""
Google Meet Productivity Agent.

This script identifies Google Meet notes (generated by Gemini) attached to calendar events
within a specified date range. It extracts action items assigned to the primary user
from the "Next Steps" or "Suggested Next Steps" section of those notes and creates 
corresponding Google Tasks.
"""
import argparse
import datetime
import logging
...
def get_default_tasklist(tasks_service):
    ...
    result = tasks_service.tasklists().list(maxResults=1).execute(num_retries=3)
...
def create_task_with_subtasks(tasks_service, tasklist_id, title, subtask_titles):
    ...
    for subtask_title in subtask_titles:
        try:
            ...
            logging.error("Failed to create subtask '%s' under '%s': %s", subtask_title, title, e)
    return parent
...
def main():
    ...
    if isinstance(primary_user, str):
    ...
    for event in events:
        ...
        for attachment in doc_attachments:
            ...
            try:
                ...
                logging.error("Could not read doc ID '%s': %s", file_id, e)
                continue
            ...
            if action_items:
                ...
                print(f"    No action items assigned to {primary_user} — skipping.")
            else:
                print("    No Next Steps section found — skipping.")

    if tasks_created:
        verb = 'would be created' if args.dry_run else 'created'
        print(f"\n{tasks_created} task(s) {verb} in Google Tasks.")
    elif next_steps_docs:
        print(f"\nFound {next_steps_docs} Next Steps section(s) but no action items assigned to you.")
    else:
        print("\nNo Next Steps sections found in any Gemini notes.")
